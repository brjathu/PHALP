...
def BoxPlusRelationalEmbedding(f_g, dim_g=80, wave_len=1000, trignometric_embedding= True):
    """
    Given a tensor with bbox coordinates for detected objects on each batch image,
    this function computes a matrix for each image
    ...
    x_min, y_min, x_max, y_max, frame_t = torch.chunk(f_g, 5, dim=-1)
...
#     delta_id = delta_id.type(delta_x)

...
class SMPLHead(nn.Module):
    ...
    def __init__(self, cfg, opt=None):
        ...
        self.cfg             = cfg
        self.opt             = opt
        npose                = 6 * (self.cfg.smpl_cfg.SMPL.NUM_BODY_JOINTS + 1)
        self.npose           = npose
        in_channels          = self.cfg.in_feat #226 #cfg.MODEL.SMPL_HEAD.IN_CHANNELS
        self.pool            = self.cfg.smpl_cfg.MODEL.SMPL_HEAD.POOL
        ...
        mean_params          = np.load(opt.MODEL.SMPL_HEAD.SMPL_MEAN_PARAMS)
    ...
    def forward(self, x, n_iter=3, smpl=False):
        ...
        for i in range(n_iter):
            xc             = torch.cat([x, pred_body_pose, pred_betas, pred_cam],1)
        ...
        pred_body_pose                     = rot6d_to_rotmat(pred_body_pose).view(batch_size, self.cfg.smpl_cfg.SMPL.NUM_BODY_JOINTS+1, 3, 3)

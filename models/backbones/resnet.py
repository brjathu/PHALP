...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, opt=None):
        self.inplanes = 64
    ...
    def forward(self, x):
        ...
        if(self.opt.mask_type=="rgb"):
            x1 = x[:, :3, :, :]
            x = self.conv1(x1)
            
        if(self.opt.mask_type=="feat"):
            x1 = x[:, :3, :, :]
            x_ = x[:, 3:, :, :]
...
def resnet(cfg, pretrained=False, num_layers=50, **kwargs):
